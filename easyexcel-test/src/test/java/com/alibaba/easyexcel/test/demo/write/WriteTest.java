...
public class WriteTest {
   ...
   * 最简单的写
   * <p>
   * 1. 创建excel对应的实体对象 参照{@link DemoExportData}
   * <p>
   * 2. 直接写即可
   */
  @Test
  public void simpleWrite() {
    ...
    // 导入的数据
    List<DemoData> importDataList = demoDataListener.cachedDataList;
    Map<String, List<DemoData>> importGroupList = importDataList.stream().collect(Collectors.groupingBy(v -> v.getPermissionName() + FILTER + v.getPermissionFeature() + FILTER + v.getRoleName()));

    // 导入的数据
    List<DemoExportData> exportData = Lists.newArrayList();

    importGroupList.forEach((k, v) -> {
      DemoExportData demoExportData = new DemoExportData();
      exportData.add(demoExportData);

      demoExportData.setPermissionName(k.split(FILTER)[0]);
      demoExportData.setPermissionFeature(k.split(FILTER)[1]);
      demoExportData.setRoleName(k.split(FILTER)[2]);

      String dataName = "";

      for (DemoData employeeVo : v) {
        if (StringUtils.isBlank(dataName)) {
          dataName = employeeVo.getEmployeeName() + "(" + employeeVo.getEmployeeWorkNumber() + ")";
        } else {
          dataName = dataName + "、" + employeeVo.getEmployeeName() + "(" + employeeVo.getEmployeeWorkNumber() + ")";
        }
      }

      demoExportData.setRoleEmployee(dataName);
    });

//    System.out.println(exportData);
    List<DemoExportData> exportData2 = Lists.newArrayList();

    Map<String, List<DemoExportData>> collect = exportData.stream().collect(Collectors.groupingBy(DemoExportData::getPermissionFeature));
    collect.forEach((k, v) -> {
      exportData2.addAll(v);
    });

    String exportFilaName = TestFileUtil.getPath() + "oa-" + System.currentTimeMillis() + ".xlsx";
    EasyExcel.write(exportFilaName, DemoExportData.class).sheet("导出结果").doWrite(exportData2);
  ...
  public void simpleRoleRelationEmployeeWrite() {
    ...
    RoleRelationEmployeeListener importVoListListener = new RoleRelationEmployeeListener();
    // 这里 需要指定读用哪个class去读，然后读取第一个sheet 文件流会自动关闭
    EasyExcel.read(fileName, RoleRelationEmployeeVo.class, importVoListListener).sheet().doRead();

    List<RoleRelationEmployeeVo> importVoList = importVoListListener.cachedDataList;
    ...
    String exportFilaName = TestFileUtil.getPath() + "role-employee" + System.currentTimeMillis() + ".xlsx";
  ...
  public void excludeOrIncludeWrite() {
    ...
    EasyExcel.write(fileName, DemoExportData.class).excludeColumnFieldNames(excludeColumnFieldNames).sheet("模板")
        .doWrite(data());
    ...
    // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
    EasyExcel.write(fileName, DemoExportData.class).includeColumnFieldNames(includeColumnFieldNames).sheet("模板")
        .doWrite(data());
  }
  ...
  public void repeatedWrite() {
    ...
    try (ExcelWriter excelWriter = EasyExcel.write(fileName).build()) {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        // 实际上可以一直变
        WriteSheet writeSheet = EasyExcel.writerSheet(i, "模板" + i).head(DemoExportData.class).build();
        // 分页去数据库查询数据 这里可以去数据库查询每一页的数据
        List<DemoExportData> data = data();
        excelWriter.write(data, writeSheet);
      }
  ...
  public void templateWrite() {
    ...
    EasyExcel.write(fileName, DemoExportData.class).withTemplate(templateFileName).sheet().doWrite(data());
  }
   ...
   * 合并单元格
   * <p>
   * 1. 创建excel对应的实体对象 参照{@link DemoExportData} {@link DemoMergeData}
   * <p>
   * 2. 创建一个merge策略 并注册
   * <p>
   * 3. 直接写即可
  ...
  public void mergeWrite() {
    ...
    // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
    EasyExcel.write(fileName, DemoExportData.class).registerWriteHandler(loopMergeStrategy).sheet("模板").doWrite(data());
  }
  ...
  public void customHandlerWrite() {
    ...
    // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
    EasyExcel.write(fileName, DemoExportData.class).registerWriteHandler(new CustomSheetWriteHandler())
  ...
  public void commentWrite() {
    ...
    EasyExcel.write(fileName, DemoExportData.class).inMemory(Boolean.TRUE).registerWriteHandler(new CommentWriteHandler())
  ...
  private List<DemoExportData> data() {
    List<DemoExportData> list = ListUtils.newArrayList();
    for (int i = 0; i < 10; i++) {
